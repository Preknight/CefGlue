...
namespace Xilium.CefGlue.Demo.Avalonia
{
    public partial class BrowserView : UserControl
    {
        ...
        private void OnBrowserWrapperKeyDown(object sender, global::Avalonia.Input.KeyEventArgs e)
        {
            if(e.Key == Key.F5)
            {
                browser.Reload();
            ...
            {
                browser.SendKeyEvent(new CefKeyEvent() { });
            }
        }
        ...
        private class BrowserLifeSpanHandler : LifeSpanHandler
        {
                ...
                ref bool noJavascriptAccess)
            {
                ...
                Dispatcher.UIThread.Post(() =>
                {
                    ...
                    if (int.IsNegative(bounds.X))
                    {
                        bounds.X = 0;
                    }
                    if (int.IsNegative(bounds.Y))
                    {
                        bounds.Y = 0;
                    }
                    if (int.IsNegative(bounds.Width))
                    {
                        bounds.Width = 800;
                    }
                    if (int.IsNegative(bounds.Height))
                    {
                        bounds.Height = 600;
                    }
